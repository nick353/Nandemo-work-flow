...
### 🟢 短時間〜中時間タスク（通常操作）
- **方法:** 同期実行（background: true 禁止）
- **待ち時間:** 1分以上でもOK（andoさん了承済み）
- **長時間の場合:** 分割して途中経過を報告
- **例:** ファイル検索、設定変更、データ処理

### 🔴 自動タスク（定期実行・cron）
- **方法:** バックグラウンド実行 + **スクリプト内で強制Discord通知**
- **条件:** スクリプトの最後に必ず `clawdbot message send` で通知
- **例:** 毎朝リサーチ（daily-research.sh）
- **メリット:** AIが忘れても、スクリプトが責任を持つ

### ❌ 禁止事項
- 「完了したら報告する」と約束してバックグラウンド実行
- AIの記憶任せの報告（必ずスクリプトで自動化）
- 長時間タスクの一括実行（分割必須）
...
---
**永続化ルール（全ての作業で必須）：**
- **一度できたことは全て永続化＆応用可能にする**
  1. **スキル化**: `/root/clawd/skills/<name>/SKILL.md` を作成
  2. **スクリプト化**: シェルスクリプト/Python など再実行可能な形式に
  3. **環境設定の永続化**: 
     - 認証情報 → `~/.profile`, 環境変数, config ファイル
     - API キー → 環境変数 + `.env` バックアップ
     - ツール設定 → `~/.config/<tool>/` + Git 管理
  4. **ドキュメント化**: 
     - TOOLS.md に使用方法・設定を記録
     - SKILL.md にコマンド例・トラブルシューティング
  5. **自動化**: cron, ハートビートタスク化（可能な場合）
- **二度手間を避ける設計原則：**
  - 手動操作 → 即座にスクリプト化
  - 調査結果 → 再利用可能なドキュメント/データベース化
  - 設定変更 → Git 管理 + 自動バックアップ
  - 成功パターン → テンプレート化
- **将来の応用を見据えた実装：**
  - 汎用的な設計（特定ケース限定禁止）
  - パラメータ化（ハードコード厳禁）
  - 拡張性（新機能追加が容易に）
  - 充実したドキュメント（未来の自分への説明）
- **永続化の確認チェックリスト：**
  - [ ] 再起動後も動作するか？
  - [ ] 他の類似タスクに応用できるか？
  - [ ] ドキュメントだけで第三者が使えるか？
