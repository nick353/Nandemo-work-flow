#!/usr/bin/env python3
"""
仮想通貨バックテスト（200 SMA/EMA重なり版）
平均足スムーズド + 200 SMA/EMA重なり + ダイバージェンス
"""
...
def check_sma_ema_overlap(sma_200, ema_200, current_price, i, threshold_pct=1.0):
...
def check_entry_conditions(df, i, ha_df, abnormal, sma_200, ema_200, macd_line, divergence_lookback=5):
    """エントリー条件チェック（200 SMA/EMA重なり版）"""
    ...
    # 200 SMA と 200 EMA が重なっているかチェック
    is_overlapping, direction = check_sma_ema_overlap(sma_200, ema_200, current_price, i, threshold_pct=1.0)
...
def run_backtest(symbol='BTC/USDT', timeframe='5m', days_back=30):
    ...
    for i in range(200, len(df)):  # 最初の200本はスキップ（SMA/EMA 200計算のため）
        ...
        if position is None:
            # エントリーチェック
            entry = check_entry_conditions(df, i, ha_df, abnormal, sma_200, ema_200, macd_line)
