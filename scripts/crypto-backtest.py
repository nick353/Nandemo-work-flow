...
def detect_abnormal_candle(ha_df, lookback=20, threshold=2.0):
    """異常値（長い実体/ヒゲ）を検出"""
...
def check_entry_conditions(df, i, ha_df, abnormal, sma_200, rsi, macd_line, divergence_lookback=5):
    """エントリー条件チェック（両方向）"""
    ...
    # 200 SMAとの位置関係
    near_sma = abs(current_price - sma_200.iloc[i]) / current_price < 0.02  # 2%以内
    ...
    # ショートエントリー条件
    if (current_price > sma_200.iloc[i] and  # 上から接触
        (abnormal['long_upper_wick'].iloc[i] or abnormal['long_bullish_body'].iloc[i]) and
        current_rsi > 70 and
        divergence == 'bearish'):
    ...
    # ロングエントリー条件
    elif (current_price < sma_200.iloc[i] and  # 下から接触
          (abnormal['long_lower_wick'].iloc[i] or abnormal['long_bearish_body'].iloc[i]) and
          current_rsi < 30 and
          divergence == 'bullish'):
...
def run_backtest(symbol='BTC/USDT', timeframe='1h', days_back=730):
    """バックテスト実行（2年間）"""
    ...
    # 異常値検出
    abnormal = detect_abnormal_candle(ha_df, lookback=20, threshold=2.0)
    ...
    for i in range(200, len(df)):  # 最初の200本はスキップ（SMA200計算のため）
        
        if position is None:
            # エントリーチェック
            entry = check_entry_conditions(df, i, ha_df, abnormal, sma_200, rsi, macd_line)
            ...
            if entry:
                ...
                print(f"📍 {entry['action'].upper()} エントリー @ {entry['price']:.2f} ({df.index[i]})")
    ...
    # 結果をJSONで保存
    result_file = f"/root/clawd/trading-strategies/backtest-result-{datetime.now().strftime('%Y%m%d-%H%M%S')}.json"
    ...
    chart_file = f"/root/clawd/trading-strategies/equity-curve-{datetime.now().strftime('%Y%m%d-%H%M%S')}.png"
    plt.savefig(chart_file)
    print(f"✅ チャート保存: {chart_file}")

if __name__ == "__main__":
    run_backtest(symbol='BTC/USDT', timeframe='1h', days_back=730)
